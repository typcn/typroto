...
void *client(){
    ...
    for (int x = 0; x < 10000 ; x++) {
        uint8_t *data_to_send = malloc(25);
        int rv = tp_send(fd, (uint8_t *)data_to_send, 25);
        if(rv != 0){
            free(data_to_send);
        }
    }
    ...
    sleep(5);
    uint64_t rs = skt->sendqueue->packet_resend;
    uint64_t sp = skt->sendqueue->packet_num;
    printf("Stats: Sent %llu, Resend: %llu\n", sp - rs, rs);
    tp_shutdown(fd);
    return NULL;
}
